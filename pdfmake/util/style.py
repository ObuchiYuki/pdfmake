...
class Color256:
    ...
    def __init__(self, n: int, is_bg: bool = False) -> None:
        if not (0 <= n <= 255):
            raise ValueError(f"Invalid 256-color number: {n}")
        self._n = n
        self.is_bg = is_bg

    def __str__(self) -> str:
        tag = "48" if self.is_bg else "38"
        return f"{tag};5;{self._n}"
...
class ColorRGB:
    ...
    def __init__(self, r: int, g: int, b: int, is_bg: bool = False) -> None:
        for k, n in {"R": r, "G": g, "B": b}.items():
            if not (0 <= n <= 255):
                raise ValueError(f"Invalid RGB color value: {k}={n}")
        self._r = r
        self._g = g
        self._b = b
        self.is_bg = is_bg
...
def styled(text: str, style: Style = []) -> str:
    if isinstance(style, list):
        for s in style:
            text = _apply_style(text, s)
        return text
    return _apply_style(text, style)


def _apply_style(text: str, style: StyleElement) -> str:
